...
with open("json_data", "w") as json_file: # Создание и последующее открытие файла с названием "json_data" в режиме "w" для записи 
    json.dump(x, json_file) # json.dump преобразует(записывает) обьект python в файл в формате json