...
val=hand()
val.__iter__() # Вызов обьекта итератора и переменной внутри класса(опять же возвращает лишь адрес итератора)
print(val.__next__()) # Вызов метода __next__
print(val.__next__())
print()

# Проделываем то же самое
h=iter(val) # Создаем обьект итератора
print(next(h)) # Переменная(обьект) играет роль итератора
print(next(h)) 